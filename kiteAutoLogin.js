...
export const performZerodhaAutoLogin = async () => {
    ...
    if (!userId || !password || !totpSecret || !apiKey || !apiSecret) {
        console.error("❌ Missing credentials in .env — check ZERODHA_USER_ID, ZERODHA_PASSWORD, ZERODHA_TOTP_SECRET, KITE_API_KEY, KITE_API_SECRET");
        process.exit(1);
    }
    ...
    try {
        ...
        while (Date.now() - start < 20000) {
            ...
            if (bodyText.includes('Invalid TOTP') || bodyText.includes('Invalid App Code')) {
                throw new Error("TOTP rejected: " + otp);
            }
        ...
        const clicked = await page.evaluate(() => {
            const btn = Array.from(document.querySelectorAll('button, a, input[type=submit]'))
                .find(el =>
                    el.innerText?.toLowerCase().includes('authoris') ||
                    el.innerText?.toLowerCase().includes('authoriz')
                );
...
// ✅ FIX: Removed process.argv check — always runs when started by PM2 or directly
