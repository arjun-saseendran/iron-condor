...
router.get('/active', async (req, res) => {
  try {
    ...
    const liveStats = trades.map(trade => {
      const { tokens, callSpreadEntryPremium, putSpreadEntryPremium, totalEntryPremium, bufferPremium, isIronButterfly, tradeType } = trade;

      // Safe fallback for 2-leg spreads
      const cp_sell = tokens.callSell ? (lastPrices[tokens.callSell] || 0) : 0;
      const cp_buy = tokens.callBuy ? (lastPrices[tokens.callBuy] || 0) : 0;
      const pp_sell = tokens.putSell ? (lastPrices[tokens.putSell] || 0) : 0;
      const pp_buy = tokens.putBuy ? (lastPrices[tokens.putBuy] || 0) : 0;

      const currentCallNet = tradeType !== 'PUT_SPREAD' ? Math.abs(cp_buy - cp_sell) : 0;
      const currentPutNet = tradeType !== 'CALL_SPREAD' ? Math.abs(pp_buy - pp_sell) : 0;
  ...
  try {
    ...
    if (!activeTrade) {
      return res.status(400).json({ 
        error: `Could not find a complete trade for today. Ensure your orders are filled.` 
      });
    }
