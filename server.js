...
import {
  initUpstoxLiveData,
  subscribeCondorSymbol,
} from "./services/upstoxLiveData.js";
...
app.get("/api/condor/positions", async (req, res) => {
  try {
    ...
    if (!activeTrade) {
      const lastTrade = await ActiveTrade.findOne({ status: "COMPLETED" }).sort(
        { updatedAt: -1 },
      );
      if (!lastTrade) return res.json([]);
      const lastPerf = await CondorPerf.findOne({
        activeTradeId: lastTrade._id,
      });
      return res.json([
        {
          status: "COMPLETED",
    ...
    const getLtp = (sym) =>
      sym ? condorPrices[kiteToUpstoxSymbol(sym, idx)] || 0 : 0;
    ...
    const totalPnL =
      (activeTrade.callSpreadEntryPremium -
        currentCallNet +
        (activeTrade.putSpreadEntryPremium - currentPutNet)) *
      activeTrade.lotSize;
    ...
    const callSL = Math.min(
      activeTrade.callSpreadEntryPremium * 4 + buffer,
      maxSpreadSL,
    );
    const putSL = Math.min(
      activeTrade.putSpreadEntryPremium * 4 + buffer,
      maxSpreadSL,
    );
    const callFirefightLevel = activeTrade.callSpreadEntryPremium * 0.3;
    const putFirefightLevel = activeTrade.putSpreadEntryPremium * 0.3;
  ...
  try {
    ...
    server.listen(PORT, async () => {
      ...
      if (process.env.UPSTOX_ACCESS_TOKEN) {
        ...
        console.warn(
          "⚠️ UPSTOX_ACCESS_TOKEN missing — Iron Condor will not receive live data",
        );
      }
        ...
        try {
          ...
          if (active) {
            ...
            [
              ...
              .filter(Boolean)
              .forEach((kite) =>
                subscribeCondorSymbol(kiteToUpstoxSymbol(kite, idx)),
              );
          }
        } catch (err) {
