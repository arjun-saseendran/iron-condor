  ...
  try {
    ...
    const findOrders = (target, type) => {
      ...
      return completedOrders.filter(order => {
        const cleanOrderSym = order.tradingsymbol.toUpperCase().replace(/\s/g, '');
        return cleanOrderSym.includes(cleanTarget) && order.transaction_type === type;
      });
    };
    ...
    const getAvg = (orders) => {
      let val = 0, qty = 0;
      orders.forEach(o => { val += (o.average_price * o.filled_quantity); qty += o.filled_quantity; });
      return val / qty;
    };
